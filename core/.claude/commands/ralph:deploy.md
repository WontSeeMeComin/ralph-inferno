...
Push projects to GitHub and launch Ralph on VM.
...
Run this validation and STOP if something is missing:
...
If something is ❌ FATAL → **STOP** and ask the user to fix it.
If something is ⚠️ WARNING → Ask if they want to continue anyway.

**STEP 2: REQUIREMENTS CHECK (if not --skip-requirements)**
...
If Claude is already working → proceed to step 4.
...
- Run ralph.sh in the background with nohup
